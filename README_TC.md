...
全部字型均為 SIL OFL 1.1 開源授權，版權與作者（霞鶩文楷 / 思源宋體 SC·TC Heavy / 馬善政）見 [licenses/FONT-LICENSES.txt](licenses/FONT-LICENSES.txt)，授權全文隨 font 與 artfont 安裝包附帶。

主選單字型透過重烘焙 TMP SDF 距離場圖集實現替換（詳見 [HACKING.md](HACKING.md)），內文透過 Font 物件 TTF 內嵌替換實現。
