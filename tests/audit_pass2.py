...
pool = [r for r in recs if r.get("en") and r.get("zh")
        and (r["id"].startswith("QUEST_") or r["id"].startswith("ENCOUNTER_"))]
print(f"二校范围 {len(pool)} 条", flush=True)
BATCH = 50
log = open("tests/pass2_log.jsonl", "a", encoding="utf-8")
rules = open("tests/pass2_rules.txt", "a", encoding="utf-8")
t0 = time.time()
TK = TO = TR = TS = 0
spent = 0.0
